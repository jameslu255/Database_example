...
class Table:
    ...
    def create_page_tail(self, col_name):
        ...
        self.free_pages_tail.append(len(self.pages) - 1)
        # self.page_directory[col_name] = Page()
        
    def update_page_tail(self, index, value):
        # update the page linked to the index
        index_relative = self.free_pages_tail[index]
        error = self.pages_tail[index_relative].write(value)
        if error == -1: # maximum size reached in page
            ...
            self.free_pages_tail[index] = len(self.pages_tail) - 1
            # self.free_pages[index].append(len(pages) - 1)
            
    def update_base_indirection(self, rid, value):
        indirection_index = self.free_pages[INDIRECTION_COLUMN]
        self.table.pages[indirection_index].set_record_byte(rid, value)
